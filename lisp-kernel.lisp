(defpackage Lisp-kernel
  (interface ()
    ...
    (defun apply (<function> &rest args &keys key-args) -> <object>)
    (defun fboundp (<symbol>) -> <boolean>)
    (defun funcall (<function> args key-args))
    ;; Control
    (defsyntax if (<boolean> <form> &opt <form>) -> <object>)
    (defsyntax progn (&body body) -> <object>)
    (defsyntax unwind-protect (<form> &body cleanup-forms) -> <object>)
    ...
    ;; Conditions
    (defsyntax let-handler (handler-bindings <form>) -> <object>)
